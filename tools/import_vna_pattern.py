...
def load_s2p_s21(path):
    """Touchstone .s2p'den (freq_hz, S21_complex) döner. '# HZ S RI R 50' formatı; sütunlar:
    ...
    for ln in open(path, encoding="utf-8", errors="ignore"):
        ...
        try:
            ...
            s21.append(complex(float(p[3]), float(p[4])))
        except ValueError:
            continue
...
def main():
    ...
    for fn in glob.glob(os.path.join(a.dir, "*.s2p")):
        ...
        if m:
            files.append((int(m.group(1)) % 360, fn))
    files.sort()
    if len(files) < 8:
    ...
    for fc in freqs:
        ...
        mag = np.mean(np.abs(S[:, mask]), axis=1)          # açı başına ortalama |S21| (doğrusal)
        grid_lin = circular_interp(angles, meas_angles, mag)
        db = 20.0 * np.log10(grid_lin + 1e-12)
